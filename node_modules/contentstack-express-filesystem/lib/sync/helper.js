...
helper.replaceAssetsUrl = function (_assets, content_type, entry) {
    // console.log("_assets in replaceAssetUrl ----------- ",_assets)
    // console.log(" content_type in replaceAssetUrl --------- ",content_type)
    // console.log(" entry in replaceassetUrl -------------- ",entry)
    if (content_type && content_type.schema && entry) {
        ...
        // console.log(" updated entry in replaceAssetUrl ----------------- ",entry)
        return entry;
    }
};
...
helper.getAssets = function () {
    ...
    return function (asset, lang, remove, cb) {
        console.log("asset in getAssets --------------- ",asset)
        try {
            ...
            // console.log(" assets in get assets ----------------------- ",asset)
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (!_.isEmpty(_assetObject) && _.isEqual(_assetObject[0], asset) && !isForceLoad && fs.existsSync(_assetPath)) {
                        ...
                        if (asset && !_.isEmpty(_assetObject) && _.isEmpty(_rteAsset)) {
                            console.log(" dummy asset ---------------- ",asset)
                            var oldAssetPath = urlFromObject(_assetObject);
                            // console.log(" oldAssetPath remove if not in rte ------------ ",oldAssetPath)
                        ...
                        //    Start to remove old asset from s3 if not referred in RTE
                            AWS.config = new AWS.Config();
                            AWS.config.accessKeyId = "AKIAI6AVGQ5Z3UMRYVLQ";
                            AWS.config.secretAccessKey = "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";
                            var bucketName = 'csassetsbucket',
                                s3 = new AWS.S3({apiVersion: '2006-03-01'}),
                                params = {Bucket: bucketName, Key:lang.code+"/"+old_uid+"/"+old_filename};
                            // console.log(" params ------------------ ",params)
                            // console.log(" lang in rte assets testing ---------------- ",lang)
                            // var s3 = new AWS.S3();
                        ...
                        asset._internal_url = _url;
                        helper.downloadAssets(_assetPath, asset, lang, function (err, data) {
                            if (err) {
                                async.setImmediate(function () {
                                    cb(err, null);
                                });
                    ...
                    if (!_.isEmpty(_assetObject)) {
                        ...
                        // console.log("_paths test asset.json --------------",_paths)
                        _paths.unshift(_path);
                        // console.log("_paths test asset.json 1 -------------",_paths)
                        var __assetPath = path.join.apply(path, _paths);
                        // console.log("__assetPath test asset.json --------- ",__assetPath)
                ...
                if (rteAssets) {
                    ...
                    var paths = [asset.uid];
                    // console.log(" rte assets path ------------- ",paths)
                    paths.unshift(_path);
                    var assetPath = path.join.apply(path, paths);
                    helper.downloadAssets(assetPath, asset, lang, function (err, data) {
                        if (!err) {
...
helper.downloadAssets = function (assetsPath, asset, lang, callback) {
    ...
    out.on('data', function (resp) {
        ...
        var data = Buffer.concat(buffer),
            bucketName = 'csassetsbucket',
            s3 = new AWS.S3({apiVersion: '2006-03-01'}),
            params = {Bucket: bucketName, Key: lang.code+"/"+asset.uid+"/"+asset.filename, ACL: 'public-read', Body: data};
        ...
        s3.putObject(params, function (err, data) {
            if (err)
                ...
                console.log("Successfully uploaded assets data in s3");
              callback(null, asset);
        });
    })
        .on('close', function () {
...
helper.unpublishAsset = function (assetUid, lang, assetPath, isRemove, callback) {
    ...
    var bucketName = 'csassetsbucket',
        s3 = new AWS.S3({apiVersion: '2006-03-01'}),
        params = {Bucket: bucketName, Key: lang.code+"/"+assetUid+"/"+filename};
    ...
    if (isRemove && fs.existsSync(assetPath)) {
        // console.log(" assetPath in if cond --------------- ",assetPath)
...
helper.deleteAssets = function (assetUid, lang, callback) {
    ...
    try {
        ...
        helper.deleteAssetFolder(assetFolderPath,assetUid,lang, function (err, data) {
            if (err) throw err;
            db
                .Assets()
                .language(lang.code)
        ...
        if (fs.existsSync(assetPath)) {
            ...
            s3.listObjects(params, function(err, data) {
                ...
                s3.deleteObjects(params, function(err, data) {
                    ...
                    return console.log("Number of object deleted from folder ---------- ",data.Deleted.length);
                });
            });
